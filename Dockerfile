...
# Compatibility-first template for cnvpytor.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/cnvpytor" ]; then BIN="/opt/conda/bin/cnvpytor"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo cnvpytor | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'cnvpytor*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/cnvpytor
RUN chmod +x /usr/local/bin/cnvpytor && rm -f /tmp/tool-entry-path
WORKDIR /data
ENTRYPOINT ["/usr/local/bin/cnvpytor"]
